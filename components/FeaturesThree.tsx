...
const FeaturesThree = () => {
  const RenderItem = ({ item }: { item: IFeatureThree }) => {
    // Check if the index is odd or even for proper alignment
    const isEven = parseInt(item.index) % 2 === 0;
      ...
      <Container className="w-full grid grid-cols-1 lg:grid-cols-2 place-items-center transition duration-300 ease-in-out px-0 lg:px-[15%]">
        <div
          className={`mt-12 lg:mt-0 pl-10 lg:pl-0 ${
            isEven ? "order-first lg:order-last" : ""
          }`}
        >
        ...
        <div className="flex flex-col items-center lg:items-start h-full w-full mt-10 lg:mt-0 gap-7 px-[10%]">
          ...
          <Link href={"/#get-started"}>
            <Button className="bg-darkBlue pb-3 pt-3 px-10 flex items-center justify-center" />
          </Link>
        </div>
      </Container>
    );
